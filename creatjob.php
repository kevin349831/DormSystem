...
$stmt = $db->query("SELECT * FROM `Student` A JOIN `Parent` B ON B.Parent_ID = A.ParentParent_ID JOIN `Class` C ON A.ClassClass_ID = C.Class_ID JOIN Department D ON D.Department_ID = C.Department_ID LEFT JOIN Job_detail E ON A.Student_ID=E.StudentStudent_ID  LEFT JOIN Job F ON E.JobJob_ID = F.Job_ID
WHERE `Class_name` LIKE '%" . $class . "%' and `Student_name` LIKE '%" . $name . "%' and `Student_number` LIKE '%" . $stu_num . "%'");
$count = 0;
$c = $stmt->fetchAll();
$per = 10; //一頁顯示幾筆資料
